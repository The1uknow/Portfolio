...
export const Hero = () => {
  ...
  return (
    ...
    >
      {/* === Навигация === */}
      <nav
        className="sticky top-0 z-50 w-full flex justify-center items-center gap-6 md:gap-10 py-4 text-sm md:text-base font-medium text-text-secondary backdrop-blur bg-background/70"
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span onClick={() => setLang("ru")} className="cursor-pointer hover:text-accent-green">
            RU
          </span>
          <span className="text-text-muted">|</span>
          <span onClick={() => setLang("en")} className="cursor-pointer hover:text-accent-green">
            EN
          </span>
          <span className="text-text-muted">|</span>
          <span onClick={() => setLang("uz")} className="cursor-pointer hover:text-accent-green">
            UZ
          </span>
        </div>
      </nav>
      ...
      <div className="relative z-10 max-w-5xl w-full mx-auto flex flex-col items-center justify-center flex-1">
        ...
        {/* === Услуги === */}
        <div className="mt-10 flex flex-wrap justify-center gap-4 md:gap-6 text-[clamp(0.8rem,2.5vw,1rem)] text-text-secondary/80 opacity-0 animate-fade-in-up">
          {services.map((s, i) => (
            <span
              key={s}
